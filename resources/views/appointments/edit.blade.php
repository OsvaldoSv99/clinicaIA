                ...
                <div class="p-6 text-gray-900">
                    <form action="{{ route('appointments.update', $appointment) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <select id="doctor_id" name="doctor_id" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                ...
                                @foreach($doctors as $doctor)
                                    <option value="{{ $doctor->id }}" {{ old('doctor_id', $appointment->doctor_id) == $doctor->id ? 'selected' : '' }}>{{ $doctor->name }} ({{ $doctor->specialty }})</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="appointment_time" class="block mt-1 w-full" type="datetime-local" name="appointment_time" :value="old('appointment_time', $appointment->appointment_time->format('Y-m-d\TH:i'))" required />
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="notes" name="notes" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('notes', $appointment->notes) }}</textarea>
